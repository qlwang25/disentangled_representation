...
from transformers.utils import colorlogging
import logging
logger = colorlogging.getLogger(name=__name__, level=logging.DEBUG)
...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    # first name has to correspond to main model input name to make sure `tokenizer.pad(...)` works correctly
    ...
    def _from_pretrained(cls, 
        ...
        config_tokenizer_class = kwargs.pop("tokenizer_class", None)
        ...
        if added_tokens:
            logger.warning("Special tokens have been added in the vocabulary, make sure the associated word embeddings are fine-tuned or trained.")
...
def get_fast_tokenizer_file(path_or_repo: Union[str, os.PathLike],
    ...
    all_files = get_list_of_files(path_or_repo, revision=revision, use_auth_token=use_auth_token, local_files_only=local_files_only)
    tokenizer_files_map = {}
