...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
        ...
        if torch_dtype is not None:
            if isinstance(torch_dtype, str):
                if torch_dtype == "auto":
                    ...
                    raise ValueError(f"`torch_dtype` can be either a `torch.dtype` or `auto`, but received {torch_dtype}")
            dtype_orig = cls._set_default_torch_dtype(torch_dtype)
    ...
    @classmethod
    def _load_state_dict_into_model(cls, model, state_dict, pretrained_model_name_or_path, ignore_mismatched_sizes=False, _fast_init=True):
